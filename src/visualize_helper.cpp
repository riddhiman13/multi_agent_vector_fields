...
void multi_agent_vector_fields::publishPathMarker(ros::Publisher& marker_pub, const std::vector<Eigen::Vector3d>& path, 
...
void multi_agent_vector_fields::publishMultiAgentPlannedPaths(ros::Publisher& marker_pub, 
                                                              ...
                                                              int best_agent_id) 
{
    for (size_t i = 0; i < predicted_paths.size(); ++i) 
    {
        ...
        if (path.size() > 2) 
        {
            publishPathMarker(marker_pub, path, 
                              static_cast<int>(i + 20), "world", "multi_agent_predicted_path", 
                              0.01, 
                              i == best_agent_id ? 1.0 : 0.0, 
                              0.2, 
                              i == best_agent_id ? 0.0 : 1.0, 
                              0.8); 
        }
    }
}
